# dsh-plugin-wallpapers installer (Windows) - BACKUP method. The recommended
# install is the npm one-liner (no account, no git, no manual steps):
#
...
#   1. get the repo (zip download, or git clone when zip fails)
#   2. create a junction in the profile's node_modules
#   3. register ui-wallpaper in cordis.patch.yml (idempotent - safe to re-run)
#
# Version: defaults to 'latest' (the newest GitHub Release, resolved via the
# API). Pin a version with -Version 'v0.1.0', or track the dev branch with
# -Version 'main'. Reload the Web UI afterwards. DSH home defaults to
# %USERPROFILE%\.dsh, override with -DshHome.

param(
    [string]$Source = 'https://github.com/TTAWDTT/dsh-plugin-wallpapers',
...
if ($isRemote) {
    ...
    if ($ref -eq 'latest' -and $repoUrl -match '^https?://github\.com/([^/]+/[^/]+)') {
        $slug = $Matches[1]
        try {
            $latest = Invoke-RestMethod -Uri "https://api.github.com/repos/$slug/releases/latest" -Headers @{ 'User-Agent' = 'dsh-wallpapers-installer' } -TimeoutSec 15
            if ($latest.tag_name) {
                $ref = $latest.tag_name
                $isTag = $true
    ...
    $zipFile  = Join-Path $pluginsDir 'wallpapers-plugin.zip'
    $extractDir = Join-Path $pluginsDir 'wallpapers-plugin-extract'
...
if (-not (Test-Path $patchFile)) {
    ...
    # Match the insert entry only, not config blocks keyed by the same id.
    if ($content -match '(?m)^\s*-\s+insert:\s*$\n(?:\s*.*\n)*?\s*-\s*id:\s*ui-wallpaper\s*$') {
        Write-Host '  already registered, skip.' -ForegroundColor DarkGray
    } elseif ($content -match '(?m)^\s*-\s+id:\s*ui-wallpaper\s*$') {
        Write-Host '  already registered, skip.' -ForegroundColor DarkGray
    } else {
...
Write-Host ''
Write-Host 'Done. Reload the Web UI, then Settings -> General -> Wallpaper.' -ForegroundColor Green
